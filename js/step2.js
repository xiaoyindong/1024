...
function setDarg() {
	...
	document.addEventListener('touchmove', function (e) {
		...
		// 翻转样式
		// document.getElementById('wanted').style.transform = 'translateX(-148px) scale(0.4) rotateY(' + nowDeg.x + 'deg)';
...
function showCard() {
	...
	wanted.className = 'wantedshow';
	var nameDom = wanted.getElementsByClassName('name')[0];
	var sexDom = wanted.getElementsByClassName('sex')[0];
	var jobDom = wanted.getElementsByClassName('job')[0];
	var speacDom = wanted.getElementsByClassName('speacl')[0];
	// var titleDom = wanted.getElementsByClassName('page_title')[0];
	var photoDom = wanted.getElementsByClassName('user_photo')[0];
	photoDom.src = window.userInfo.avatar || 'http://tx.haiqq.com/uploads/allimg/170826/04451MY6-2.jpg'
	photoDom.ontouchend = function () {
		// location.href = '1024.html';
	}
	setTimeout(() => {
		// location.href = '1024.html';
	}, 5000);
	var name = window.userInfo.name || '匿名用户'
	var sex = getBirthDate(window.userInfo.hiredDate) || '未知'
	var job = window.userInfo.position || '神秘黑客'
	var speac = window.speac || '未知';
	function renderText(dom, text, num) {
		dom.innerText = text.substring(0, num);
		if (num >= text.length) {
			return
		}
		setTimeout(function () {
			num++;
			renderText(dom, text, num);
		}, 180)
	};
	setTimeout(function () {
		renderText(nameDom, name, 1);
	}, 500);

	setTimeout(function () {
		renderText(sexDom, sex, 1);
	}, 600);
	setTimeout(function () {
		renderText(jobDom, job, 1);
	}, 700);
	setTimeout(function () {
		renderText(speacDom, speac, 1);
	}, 800);
